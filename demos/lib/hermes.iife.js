var Hermes = (function (exports) {
    ...
    const idempotentLogNumber = (base, max, min, count, index, options = {}) => {
    ...
    const idempotentNumber = (max, min, count, index, options = {}) => {
        var _a, _b, _c;
        const adjustedCount = count > 1 ? count - 1 : 1;
        const inc = (max - min) / adjustedCount;
        if (Object.keys(options).length !== 0) {
            const percentNaN = (_a = options.includeNaN) !== null && _a !== void 0 ? _a : 0;
            const percentNegInfinity = ((_b = options.includeNegativeInfinity) !== null && _b !== void 0 ? _b : 0) + percentNaN;
            const percentPosInfinity = ((_c = options.includePositiveInfinity) !== null && _c !== void 0 ? _c : 0) + percentNegInfinity;
            const percent = index / count;
            if (percent <= percentNaN)
                return Number.NaN;
            else if (percent <= percentNegInfinity)
                return Number.NEGATIVE_INFINITY;
            else if (percent <= percentPosInfinity)
    ...
    class LinearScale extends NiceScale {
        constructor(direction, finiteMin, finiteMax, actualMin, actualMax, config = {}) {
    ...
    const generateData = (dimensions, count, random = true, options = {}) => {
        return dimensions.reduce((acc, dimension) => {
            acc[dimension.key] = new Array(count).fill(null).map((_, index) => {
                if (dimension.type === DimensionType.Categorical) {
                    ...
                    if (range) {
                        return random
                            ? randomNumber(range[1], range[0], options)
                            : idempotentNumber(range[1], range[0], count, index, options);
                    }
                }
                    ...
                    if (range && dimension.logBase) {
                        return random
                            ? randomLogNumber(dimension.logBase, range[1], range[0], options)
    ...
    class Hermes {
        ...
        setDimensions(dimensions, redraw = true) {
            ...
            this.dimensions = clone(dimensions).map(dimension => {
                ...
                if (dimension.type === DimensionType.Linear ||
                    ...
                    if (dimension.type === DimensionType.Linear) {
                        internal.scale = new LinearScale(direction, range.finite[0], range.finite[1], range.actual[0], range.actual[1], dimension);
                    }
                    else if (dimension.type === DimensionType.Logarithmic) {
        ...
        setSize(w, h, redraw = true) {
            ...
            // Make hook callback.
            (_b = (_a = this.config.hooks).onResize) === null || _b === void 0 ? void 0 : _b.call(_a, clone(this.size), oldSize);
            if (redraw)
                this.redraw();
        }
        disable() {
        ...
        updateActiveLabel() {
            ...
            if (newIndex !== -1) {
                ...
                // Make hook callback.
                (_b = (_a = this.config.hooks).onDimensionMove) === null || _b === void 0 ? void 0 : _b.call(_a, clone(dragDimension[0]), newIndex, oldIndex);
            }
        }
        ...
        updateActiveFilter(e) {
            ...
            if (distance(_ixsa.p0, _ixsa.p1) < FILTER_REMOVE_THRESHOLD) {
                ...
                if (removeIndex !== -1) {
                    // Make hook callback.
                    const filter = clone(filters[removeIndex]);
                    (_d = (_c = this.config.hooks).onFilterRemove) === null || _d === void 0 ? void 0 : _d.call(_c, { [dimensionKey]: [filter] });
                    // Remove filter.
                ...
                const filters = { [dimensionKey]: [clone(_ixf.active)] };
                // Make corresponding filter hook callback.
                switch (_ixsa.type) {
                    case ActionType.FilterCreate:
                        (_f = (_e = this.config.hooks).onFilterCreate) === null || _f === void 0 ? void 0 : _f.call(_e, filters);
                        break;
                    case ActionType.FilterMove:
                        (_h = (_g = this.config.hooks).onFilterMove) === null || _h === void 0 ? void 0 : _h.call(_g, filters);
                        break;
                    case ActionType.FilterResizeAfter:
                    case ActionType.FilterResizeBefore:
                        (_k = (_j = this.config.hooks).onFilterResize) === null || _k === void 0 ? void 0 : _k.call(_j, filters);
                        break;
                }
            }
            ...
            (_o = (_m = this.config.hooks) === null || _m === void 0 ? void 0 : _m.onFilterChange) === null || _o === void 0 ? void 0 : _o.call(_m, clone(this.filters));
        }
        getDimensionLayoutInfo(dimIndex) {
            if (!this._ || !this.dataInfo)
                return;
        ...
        processConfigFilter(filter, dimIndex) {
            const i = this.getDimensionLayoutInfo(dimIndex);
            if (!i)
                return;
                ...
                if (i.hasInfinity) {
                    if (i.isHorizontal) {
                        if (filter.hasPositiveInfinity) {
                            possibleP0.push(0.0);
                            possibleP1.push(0.0 + FILTER_EPSILON);
                        }
                        else if (filter.hasNegativeInfinity) {
                            const pi = (i.layout.axisInfinityStop.y - i.layout.axisBoundaryStart.y) / i.yLength;
                            possibleP0.push(pi - FILTER_EPSILON);
                            possibleP1.push(pi + FILTER_EPSILON);
                        }
                    }
                    else {
                        if (filter.hasPositiveInfinity) {
                            possibleP0.push(1.0 - FILTER_EPSILON);
                            possibleP1.push(1.0);
                        }
                        else if (filter.hasNegativeInfinity) {
                            const pi = (i.layout.axisInfinityStart.x - i.layout.axisBoundaryStart.x) / i.xLength;
        ...
        draw() {
            ...
            for (let k = 0; k < this.dataInfo.seriesCount; k++) {
                ...
                if ((_a = dataStyle.series) === null || _a === void 0 ? void 0 : _a[k])
                    dataDefaultStyle = (_b = dataStyle.series) === null || _b === void 0 ? void 0 : _b[k];
